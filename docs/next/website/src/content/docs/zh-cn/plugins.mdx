...
默认情况下,弹窗会接收包括 Escape 在内的所有终端输入,并在命令退出或发送 `popup.close` 请求时关闭。
小于弹窗最小尺寸的值会限制为最小值。

当弹窗只用于显示时,只能在弹窗入口点设置 `input_passthrough = true`。
键盘和粘贴输入（包括协商后的键盘协议事件）会继续发送到已聚焦的工作区窗格。
弹窗可见时会消费指针输入,因此不会更改下方工作区或打开第二个模态界面。
弹窗仍是会话级模态,并会在命令退出或收到 `popup.close` 时关闭。
