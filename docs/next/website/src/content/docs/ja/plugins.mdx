...
デフォルトでは Escape を含むすべてのターミナル入力を受け取り、コマンドが終了するか `popup.close` リクエストが送られると閉じます。
最小サイズより小さい寸法は最小値に制限されます。

ポップアップを表示専用にする場合は、ポップアップのエントリーポイントでのみ `input_passthrough = true` を設定します。
キーボード入力、貼り付け入力、およびネゴシエートされたキーボードプロトコルのイベントは、フォーカスされているワークスペースペインに送られます。
ポップアップ表示中のポインター入力は消費されるため、下にあるワークスペースを変更したり別のモーダルを開いたりすることはできません。
ポップアップはセッションモーダルのままで、コマンド終了または `popup.close` で閉じます。
